...
const BookingModal: React.FC<BookingModalProps> = ({ bus, serviceMenuEmbedCode, onClose }) => {
  ...
  // Meta Pixel: fire a Lead event whenever a booking modal is opened.
  // Uses crypto.randomUUID() as the eventID so the pixel and any future
  // server-side conversion event can be deduped on Meta's side.
  useEffect(() => {
    const fbq = (window as any).fbq;
    if (typeof fbq === 'function') {
      fbq('track', 'Lead', { content_name: bus?.name || 'Party Bus Fleet' }, { eventID: crypto.randomUUID() });
    }
  }, []);
