  ...
  return (
    <section className="relative w-full h-screen min-h-[700px] lg:min-h-[800px] overflow-hidden bg-gray-900">
      ...
      <div className="relative z-10 h-full flex flex-col justify-center items-center text-center px-4 sm:px-6">
        <div className={`max-w-5xl mx-auto transition-all duration-1000 ease-out lg:pt-24 ${
          isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
        }`}>
          {/* Location badge with glow effect - mobile only */}
          <div className="inline-block lg:hidden text-base sm:text-xl md:text-2xl font-bold uppercase tracking-[0.25em] sm:tracking-[0.35em] text-[#b9ff66] mb-6 sm:mb-8 drop-shadow-[0_0_15px_rgba(57,255,20,0.5)]">
            {"Houma • Thibodaux • New Orleans".split('').map((letter, index) => (
              <span
                key={index}
                className="inline-block animate-wave-cascade"
                style={{
                  animationDelay: `${index * 30}ms`, // Slightly faster for mobile
                  opacity: 0
                }}
              >
                {letter === ' ' ? '\u00A0' : letter}
          ...
          {/* Location badge for desktop - shown right after TIMES ROLL with wave animation */}
          <div className="hidden lg:block text-2xl font-bold uppercase tracking-[0.35em] text-[#b9ff66] mb-8 drop-shadow-[0_0_15px_rgba(57,255,20,0.5)]">
            {"Houma • Thibodaux • New Orleans".split('').map((letter, index) => (
              <span
                key={index}
                className="inline-block animate-wave-cascade"
                style={{
                  animationDelay: `${(index + 22) * 40}ms`, // Start after "TIMES ROLL" (12 letters + 10 for "LET THE GOOD ")
                  opacity: 0
                }}
              >
                {letter === ' ' ? '\u00A0' : letter}
          ...
          <p className="max-w-2xl mx-auto text-lg sm:text-xl md:text-2xl lg:text-3xl text-gray-100 font-medium mb-10 sm:mb-14 drop-shadow-lg px-4 leading-relaxed animate-subtitle-fade"
      ...
      <style>{`
        ...
        .animate-wave-cascade {
          animation: wave-cascade 0.6s cubic-bezier(0.68, -0.55, 0.265, 1.55) forwards;
          transform-origin: center bottom;
        }

        .animate-subtitle-fade {
          animation: subtitle-fade 0.8s ease-out forwards;
        }

        .animate-buttons-slide {
          animation: buttons-slide 0.8s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
        }

        .animate-trust-pop {
          animation: trust-pop 0.6s cubic-bezier(0.68, -0.55, 0.265, 1.55) forwards;
        }
      `}</style>
    </section>
  );
